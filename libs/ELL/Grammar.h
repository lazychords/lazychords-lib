...
struct GrammarBase
{
    ...
    template <typename T>
    Ch<Token>                          ch(const T & t) const
    {
        return Ch<Token>((Token) t);
    }
...
#       undef ELL_FLAG
    ...
    SSk<Token, P, SK>                  skip(const P & p, const SK & skipper)
    {
        return SSk<Token, P, SK>(p, skipper);
    }

    template <typename P>
    NCs<Token, P>                      no_consume(const P & p) const
    {
        return NCs<Token, P>(p);
    }

    template <typename P>
    Lx<Token, P>                       lexeme(const P & p) const
    {
        return Lx<Token, P>(p);
    }

    template <const int exact, typename P>
    Rp<Token, P, exact, exact>         repeat(const P & p) const
    {
        return Rp<Token, P, exact, exact>(p);
    }

    template <const int min, const int max, typename P>
    Rp<Token, P, min, max>             repeat(const P & p) const
    {
        return Rp<Token, P, min, max>(p);
    }

    template <typename P, typename CP>
    DRp<Token, P, CP>                  repeat(const int CP::*exact, const P & p) const
    {
        return DRp<Token, P, CP>(p, exact, exact);
    }

    template <typename P, typename CP>
    DRp<Token, P, CP>                  repeat(const int CP::*min, const int CP::*max, const P & p) const
    {
        return DRp<Token, P, CP>(p, min, max);
    }

    template <typename P, typename Suffix>
    NSx<Token, P, Suffix>              no_suffix(const P & p, const Suffix & s) const
    {
        return NSx<Token, P, Suffix>(p, s);
    }

    Err<Token>                         error(const std::string & msg) const
    {
        return Err<Token>(msg);
    }
};
...
struct CharGrammarBase : public GrammarBase<Token>
{
    ...
    integer() const
    {
        return Int<Token, Sign, Radix, MinDigits, MaxDigits>();
    }
    ...
    Rg<Token, C1, C2>                   range() const
    {
        return Rg<Token, C1, C2>();
    }

    ChS<Token>                          chset(const std::string & set) const
    {
        return ChS<Token>(set);
    }

    Str<Token>                          str(const std::basic_string<Token> & arg) const
    {
        return Str<Token>(arg);
    }

    IStr<Token>                         istr(const std::basic_string<Token> & arg) const
    {
        return IStr<Token>(arg);
    }

    Kw<Token>                           kw(const std::basic_string<Token> & s) const
    {
        return Kw<Token>(s);
    }

    IKw<Token>                          ikw(const std::basic_string<Token> & s) const
    {
        return IKw<Token>(s);
    }
};
